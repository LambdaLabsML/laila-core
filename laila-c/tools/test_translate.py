...
import laila_translate as L  # noqa: E402
...
def compile_link_run(cpp: str, timeout: int = 25):
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        cc = L._compiler()
        comp = subprocess.run(
            [cc, "-std=c++17", "-I", L._INCLUDE, "-I", L._HAL_INCLUDE, src,
             "-Wl,--start-group", _CORE, _HALP, "-Wl,--end-group", "-pthread", "-o", out],
...
for _sym in ["RememberOpts", "get_active_policy", "activate_policy", "add_tcpip_peer",
             "add_connection", "LailaValue", "from_string", "Future", "CentralCommunication",
             "Entry", "->data()", "RemotePolicyProxy"]:
    reg(f"anchor/has:{_sym}", (lambda s: (lambda: ok(s in L.api_anchor(), f"anchor missing {s}")))(_sym))
...
for _n, _inp, _want in _FENCE_CASES:
    reg(f"fences/{_n}", (lambda i, w: (lambda: eq(L._strip_fences(i).strip(), w, "fence")))(_inp, _want))
...
for _hdr in ["esp_wifi.h", "freertos/FreeRTOS.h", "lwip/sockets.h", "driver/gpio.h", "nvs_flash.h", "esp_netif.h"]:
    def _mk(h):
        def f():
...
# ========================================================== COMPILE-GATE: GOOD
_GOOD = [
    ("constant-string", 'auto e=laila->constant(LailaValue::from_string("x")); laila->memorize(e)->wait(); (void)laila->remember(e->global_id())->data();'),
    ...
    ("constant-bytes", 'std::vector<uint8_t> b{1,2,3}; auto e=laila->constant(LailaValue::from_bytes(b)); (void)e;'),
    ("constant-json", 'Json o=Json::object(); o["k"]=std::string("v"); auto e=laila->constant(LailaValue::from_json(o)); (void)e;'),
    ("variable", 'auto e=laila->variable(); (void)e;'),
    ("forget", 'auto e=laila->constant(LailaValue::none()); laila->memorize(e)->wait(); laila->forget(e->global_id())->wait();'),
    ("build", 'auto e=laila->constant(LailaValue::from_string("x")); laila->build(e)->wait();'),
    ("remember-opts-persist", 'RememberOpts o; o.persist=false; auto e=laila->constant(LailaValue::from_string("x")); laila->memorize(e)->wait(); (void)laila->remember(e->global_id(),o);'),
    ("remember-pool_nickname", 'RememberOpts o; o.pool_nickname=std::string("store"); auto f=laila->remember("LAILA:ENTRY:GLOBAL_ID:x",o); (void)f;'),
    ("remember-policy_id", 'RememberOpts o; o.policy_id=std::string("peer"); auto f=laila->remember("g",o); (void)f;'),
    ("memorize-group", 'std::vector<EntryPtr> v{laila->constant(LailaValue::from_int(1))}; laila->memorize(v)->wait();'),
    ("add_peer", 'auto id=laila->add_peer("tcp://h:1","s"); (void)id;'),
    ("add_tcpip_peer", 'auto id=laila->add_tcpip_peer("h",(uint16_t)1,"s"); (void)id;'),
    ("serve-add-connection", 'auto _tcp=std::make_shared<DefaultTCPIPProtocol>(std::string("0.0.0.0"),(uint16_t)0,std::string("s")); laila->add_connection(_tcp); (void)_tcp->bound_port();'),
    ("request", 'Json a=Json::array(); a.push_back(Json("x")); auto f=laila->request("peer","central.memory.remember",a); (void)f;'),
    ("peer-proxy", 'auto p=laila->peer("x"); (void)p;'),
    ("entry-as_dict", 'auto e=Entry::constant(LailaValue::from_string("x")); auto d=e->as_dict(); (void)d;'),
    ("future-data", 'auto e=laila->constant(LailaValue::from_string("x")); laila->memorize(e)->wait(); auto v=laila->remember(e->global_id())->data(); (void)v.as_string();'),
    ("future-result-gid", 'auto e=laila->constant(LailaValue::from_int(1)); auto f=laila->memorize(e); f->wait(); (void)f->result()->global_id();'),
    ("tensor", 'std::vector<uint8_t> raw{0,0}; auto v=LailaValue::tensor(raw,"int8",{2}); auto e=laila->constant(v); (void)e;'),
    ("entry-evolve", 'auto e=laila->constant(LailaValue::from_int(1)); auto e2=e->evolve(LailaValue::from_int(2)); (void)e2;'),
    ("serialize-roundtrip", 'auto e=Entry::constant(LailaValue::from_string("x")); auto d=e->serialize(); auto e2=Entry::from_dict(d); (void)e2;'),
    ("set-namespace", 'laila->set_active_namespace("ns"); auto e=laila->constant(LailaValue::none()); (void)e;'),
]
...
_BAD = [
    ...
    ("value-to_string", 'auto e=laila->constant(LailaValue::from_string("x")); auto s=LailaValue::none().to_string(); (void)s;'),
...
for _k in ["CMakeLists.txt", "sdkconfig.defaults", "main/CMakeLists.txt", "main/wifi.cpp", "README.md"]:
...
def _():
    ...
    eq(rc, 0); ok('"r"' in out and "42" in out, out)
...
def _():
    ...
    rc, out = compile_link_run(_wrap(
        ...
        ' std::printf("PEER=%s\\n", v.as_string().c_str());'))
    eq(rc, 0, f"out={out}"); ok("PEER=frompeer" in out, out)
...
def _run_live(target, body):
    ...
    cfg = {"project": "t", "wifi_ssid": "s", "wifi_pass": "p",
           "peer_host": "10.0.0.5", "peer_port": 8770, "peer_secret": "s"}
...
for _n, _body in _PY_CORPUS[:4]:
    def _mk_esp(b):
        def f():
            ...
            tgt, t_ok = L.verify_and_fix(token, model, tgt, ["-DLAILA_SINGLE_CORE=1"],
                                         "c2target(esp32)", 2, host_check=True)
